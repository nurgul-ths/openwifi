...
#   File "/home/andreas/gitlab_wisense/devices/openwifi_boards/run_exp_multiprocessing.py", line 15, in <module>
#     from owpy.apps.run import run
#   File "/home/andreas/gitlab_wisense/devices/openwifi_boards/owpy/apps/run.py", line 8, in <module>
#     from owpy.apps.capture_iq_app_udp import iq_capture_app_udp
#   File "/home/andreas/gitlab_wisense/devices/openwifi_boards/owpy/apps/capture_iq_app_udp.py", line 17, in <module>
#     from owpy.capture.data_parsers import *
#   File "/home/andreas/gitlab_wisense/devices/openwifi_boards/owpy/capture/data_parsers.py", line 471
#     save_complex_data(fd_dict[f"{key.upper()}_REAL}_fd"], fd_dict[f"{key.upper()}_IMAG}_fd"], data_dict[key][trans, :])
#                                                        ^
# SyntaxError: f-string: single '}' is not allowed
# make: *** [Makefile:32: init] Error 1
...
# DMA Symbol 0: Timestamp (64-bit)
# DMA Symbol 1:
#     - Bits 63-32: 32-bit frequency offset
#     - Bits 31-3: 29-bit LO frequency
#     - Bits 2-0: 3 bits reserved
# DMA Symbol 2-7: Additional timestamps (64-bit each)
# DMA Symbol 8-63: CSI Data (56 DMA symbols)
# DMA Symbol 64+: Equalizer Data ((56-4) DMA symbols * num_eq times)
...
# ---------------
# DMA Symbol 0: Timestamp (64-bit)
#     We sample the signal tsf_val_lock_by_iq_trigger which is a 100 MHz clock signal
#     sampled by the IQ trigger signal.
# DMA Symbol 1: Meta data
#     - Bit 63: IQ/CSI indicator (1 for IQ data, 0 for CSI data)
#     - Bit 62: iq_trigger_source_multistatic
#     - Bit 61: iq_capture_all_antenna
#     - Bits 60-32 (29 bits): lo_frequency in deca Hz
#     - Bits 31-0: Reserved
# DMA Symbol 2: Reserved
# DMA Symbol 3+: I/Q data
#     For each 64-bit DMA symbol, we have 4 16-bit values. This allows us to send data from
#     2 antennas at the same time. The I/Q data is stored as follows:
#     - When iq_capture_all_antenna is off:
#         [I_RX0, Q_RX0, I_RX1, Q_RX1]
#     - When iq_capture_all_antenna is on:
#         [RX0/RX1 data (iq_len DMA symbols)] [0xBADC0FFEE0DDF00D (1 DMA symbol)] [TX data (iq_len/2 DMA symbols)]
#     Note: When capturing TX data, a byte enable is used to selectively write the TX data
#     in 32-bit chunks at a time into 64-bit DMA symbols.
...
# # General constants
# BITS_PER_BYTE = 8
# BYTES_PER_DMA_SYMBOL = 8
# WORDS_PER_DMA_SYMBOL = 4  # 64-bit DMA symbol contains 4 16-bit words

# # CSI-specific constants
# CSI_HEADER_LEN_DMA_SYM = 8
# CSI_LEN_DMA_SYM        = 56
# EQUALIZER_LEN_DMA_SYM  = 52
# CSI_LEN_HALF_DMA_SYM   = CSI_LEN_DMA_SYM // 2
# CSI_DATA_TYPE          = 0

# # IQ-specific constants
# IQ_HEADER_LEN_DMA_SYM   = 3
# IQ_CSI_INDICATOR_BIT    = 63
# TRIGGER_SRC_BIT         = 62
# CAPTURE_ALL_ANT_BIT     = 61
# IQ_DATA_TYPE            = 1

# # Shared constants
# MAX_NUM_DMA_SYM         = 8192
# LO_FREQ_BIT_WIDTH       = 29
# LO_FREQ_START_BIT       = 32

# # Indices in 16-bit words
# CSI_CAPTURE_TIMESTAMP_IDX  = 0 * WORDS_PER_DMA_SYMBOL
# CSI_FREQ_OFFSET_EST_IDX    = 1 * WORDS_PER_DMA_SYMBOL
# CSI_CAPTURE_LO_FREQ_IDX    = 1 * WORDS_PER_DMA_SYMBOL
# CSI_CAPTURE_LO_FREQ_OFFSET = 32
# TIMESTAMPS_EXTRA_IDX       = 2 * WORDS_PER_DMA_SYMBOL

# IQ_CAPTURE_TIMESTAMP_IDX      = 0 * WORDS_PER_DMA_SYMBOL
# IQ_CAPTURE_LO_FREQ_IDX        = 1 * WORDS_PER_DMA_SYMBOL
# IQ_CAPTURE_LO_FREQ_OFFSET     = 32
# IQ_CAPTURE_ALL_ANTENNA_IDX    = 1 * WORDS_PER_DMA_SYMBOL
# IQ_CAPTURE_ALL_ANTENNA_OFFSET = 61
# IQ_CAPTURE_TRIGGER_SRC_IDX    = 1 * WORDS_PER_DMA_SYMBOL
# IQ_CAPTURE_TRIGGER_SRC_OFFSET = 62
# IQ_CAPTURE_METADATA_IDX       = 2 * WORDS_PER_DMA_SYMBOL
# IQ_CAPTURE_IQ_IDX             = 3 * WORDS_PER_DMA_SYMBOL

# # Data type detection
# DATA_TYPE_LIST           = {'csi': CSI_DATA_TYPE, 'iq': IQ_DATA_TYPE}
# DATA_BIT_DMA_SYMBOL_IDX  = 1
# DATA_BIT_BYTE_IDX        = 7
...
# def get_uint64(data, start_idx):
#   """Reconstructs a 64-bit unsigned int from 4 16-bit values.

#   Args:
#     data (numpy.ndarray): Array of 16-bit unsigned integers.
#     start_idx (int): Starting index in the data array.

#   Returns:
#     numpy.ndarray: Array of reconstructed 64-bit unsigned integers.
#   """
#   return np.sum(data[:, start_idx:start_idx+WORDS_PER_DMA_SYMBOL] * (2**np.arange(0, 64, 16)), axis=1)

# def get_uint32(data, start_idx):
#   """Reconstructs a 32-bit unsigned int from 2 16-bit values.
...
#   Returns:
#     numpy.ndarray: Array of reconstructed 32-bit unsigned integers.
#   """
#   return np.sum(data[:, start_idx:start_idx+2] * (2**np.arange(0, 32, 16)), axis=1)

# def extract_bit(value, bit_position):
#   """Extracts a single bit from a value at the specified position.

#   Args:
#     value (int): The value to extract the bit from.
#     bit_position (int): The position of the bit to extract (0-indexed).
...
# def extract_bits(value, start_bit, num_bits):
#   """Extracts a range of bits from a value.

#   Args:
#     value (int): The value to extract bits from.
...
#       - numpy.ndarray: Reshaped side information.
#       - int: Number of frames.
#       - int: Number of 16-bit integers per transmission.
#   """
#   num_dma_symbol_per_trans = CSI_HEADER_LEN_DMA_SYM + CSI_LEN_DMA_SYM + num_eq*EQUALIZER_LEN_DMA_SYM
#   num_int16_per_trans      = num_dma_symbol_per_trans * WORDS_PER_DMA_SYMBOL
#   num_frames               = len(side_info_uint16) // num_int16_per_trans
...
#   Returns:
#     tuple: A tuple containing:
#       - dict: Parsed CSI data including frequency offset, CSI, and equalizer data.
#       - dict: Timestamps keyed by their names.
#       - int: LO frequency.
#       - int: Number of frames.
#   """
#   side_info_uint16_reshaped, num_frames, num_uint16_per_trans = reshape_csi_side_info16(side_info_uint16, num_eq)
...
#   # REVISIT: This is wrong, the frequency offset is 32-bits in hardware
#   freq_offset = (20 * MHZ_TO_HZ * side_info_int16_reshaped[:, CSI_FREQ_OFFSET_EST_IDX] / 512) / (2 * np.pi)
...
#   csi = np.zeros((num_frames, CSI_LEN_DMA_SYM), dtype='complex64')

#   for i in range(num_frames):
#     tmp_vec_i = side_info_int16_reshaped[i, WORDS_PER_DMA_SYMBOL*CSI_HEADER_LEN_DMA_SYM     : (num_uint16_per_trans - 1) : WORDS_PER_DMA_SYMBOL]
#     tmp_vec_q = side_info_int16_reshaped[i, WORDS_PER_DMA_SYMBOL*CSI_HEADER_LEN_DMA_SYM + 1 : (num_uint16_per_trans - 1) : WORDS_PER_DMA_SYMBOL]
#     tmp_vec   = tmp_vec_i + 1j*tmp_vec_q

#     # Rearrange CSI data from [DC, positive freq, negative freq] to [negative freq, DC, positive freq]
#     csi[i, :CSI_LEN_HALF_DMA_SYM]  = tmp_vec[CSI_LEN_HALF_DMA_SYM : CSI_LEN_DMA_SYM]
#     csi[i,  CSI_LEN_HALF_DMA_SYM:] = tmp_vec[0 : CSI_LEN_HALF_DMA_SYM]
...
#   Args:
#     side_info_uint16_reshaped (numpy.ndarray): Reshaped side information data.
#     csi_timestamp_names (list): List of CSI timestamp names.
...
#   # Process remaining timestamps after the first one (range starts from 1) and after the freq offset (+2)
#   for i, name in enumerate(csi_timestamp_names[1:], start=1):
#     dma_sym_idx   = i + (TIMESTAMPS_EXTRA_IDX // WORDS_PER_DMA_SYMBOL)
#     timestamp     = get_uint64(side_info_uint16_reshaped, WORDS_PER_DMA_SYMBOL*dma_sym_idx)
#     timestamp_dict[name] = timestamp

#   # Get the LO frequency (carrier frequency, not the carrier frequency offset)
#   csi_header      = get_uint64(side_info_uint16_reshaped, CSI_FREQ_OFFSET_EST_IDX)
#   lo_freq_deca_hz = extract_bits(csi_header, CSI_CAPTURE_LO_FREQ_OFFSET, LO_FREQ_BIT_WIDTH)
#   lo_freq         = lo_freq_deca_hz * DECA_HZ_TO_HZ

#   return timestamp_dict, lo_freq

# #==============================================================================
# # IQ PARSING FUNCTIONS
# #==============================================================================
...
#   Reshape and segment the I/Q data from the buffer into a 2D array of I/Q values,
#   with in-phase values at lower bits and quadrature values at higher bits.
#   The first axis has the number of transactions and the second axis has the number of 16-bit values per transaction.
...
#   Returns:
#     numpy.ndarray: The reshaped buffer.

#   Raises:
#     ValueError: If the buffer cannot be reshaped as expected.
#   """
#   num_int16_per_trans = num_dma_symbol_per_trans * WORDS_PER_DMA_SYMBOL
#   num_frames          = len(buffer) // num_int16_per_trans
...
#   Args:
#     buffer_uint16 (numpy.ndarray): The buffer containing the raw I/Q data (parsed as unsigned).
#     num_dma_symbol_per_trans (int): Number of DMA symbols per transaction.
#     iq_len (int): The length of the I/Q data.
#     data_type (str): The type of I/Q data being parsed.

#   Returns:
#     tuple: A tuple containing:
#       - numpy.ndarray: The extracted timestamp.
#       - numpy.ndarray: LO frequency.
#       - numpy.ndarray: Trigger source.
#       - numpy.ndarray: Capture all antenna flag.
#       - int or None: TX start length if capture_all_antenna is True, else None.
#   """
#   buffer_uint16_reshaped = reshape_iq_side_info16(buffer_uint16, num_dma_symbol_per_trans)
...
#   # Metadata: Extract LO frequency, trigger source, and capture_all_antenna flag
#   iq_header = get_uint64(buffer_uint16_reshaped, IQ_CAPTURE_LO_FREQ_IDX)
#   lo_freq = extract_bits(iq_header, LO_FREQ_START_BIT, LO_FREQ_BIT_WIDTH) * DECA_HZ_TO_HZ
#   trigger_src = extract_bit(iq_header, TRIGGER_SRC_BIT)
#   capture_all_antenna = extract_bit(iq_header, CAPTURE_ALL_ANT_BIT)

#   if data_type == 'iq_all' and np.any(capture_all_antenna == 0):
#     print('Warning: capture_all_antenna is off in extracted data, but data_type set for experiment is iq_all.')

#   # Calculate TX start length when capturing all antennas
#   tx_start_len = IQ_CAPTURE_IQ_IDX // WORDS_PER_DMA_SYMBOL + iq_len + 2 if capture_all_antenna else None

#   return timestamp, lo_freq, trigger_src, capture_all_antenna, tx_start_len

# def parse_iq(data_type, buffer_uint16, num_dma_symbol_per_trans, iq_len, tx_start_len = None):
#   """
#   Parse I/Q data from buffer for different antenna configurations.

#   Args:
#     data_type (str): The type of I/Q data to parse ('rx_iq0_iq1', 'tx_rx_iq0', or 'iq_all').
#     buffer_uint16 (numpy.ndarray): The buffer containing the raw I/Q data.
#     num_dma_symbol_per_trans (int): The number of DMA symbols per transaction.
#     iq_len (int): The length of the I/Q data.
#     tx_start_len (int, optional): Index where TX data starts when capture_all_antenna is True.

#   Returns:
#     dict: A dictionary containing the parsed I/Q data for different antennas.
#   """
#   buffer_reshaped = reshape_iq_side_info16(buffer_uint16, num_dma_symbol_per_trans)
...
#   iq0_capture = buffer_reshaped[:, (IQ_CAPTURE_IQ_IDX+0)::WORDS_PER_DMA_SYMBOL] + 1j*buffer_reshaped[:, (IQ_CAPTURE_IQ_IDX+1)::WORDS_PER_DMA_SYMBOL]
#   iq1_capture = buffer_reshaped[:, (IQ_CAPTURE_IQ_IDX+2)::WORDS_PER_DMA_SYMBOL] + 1j*buffer_reshaped[:, (IQ_CAPTURE_IQ_IDX+3)::WORDS_PER_DMA_SYMBOL]

#   # Trim the data for 'iq_all' mode
#   if data_type == 'iq_all':
#     iq0_capture = iq0_capture[:, :iq_len]
#     iq1_capture = iq1_capture[:, :iq_len]

#     if tx_start_len is None:
#       print('Warning: When capture_all_antenna is True, tx_start_len must be provided to find the index where the TX starts')
#     else:
#       iq_tx_capture = buffer_reshaped[:, WORDS_PER_DMA_SYMBOL*tx_start_len::2] + 1j*buffer_reshaped[:, WORDS_PER_DMA_SYMBOL*tx_start_len+1::2]
#       if iq_tx_capture.shape[1] != 2*(iq_len//2)-2:
#         print(f"Warning: iq_tx_capture.shape[1] does not match {2*(iq_len//2)-2}")
#         print("iq_tx_capture.shape[1]:", iq_tx_capture.shape[1])
...
#   if data_type == 'rx_iq0_iq1':
#     data_dict = {'rx0': iq0_capture, 'rx1': iq1_capture, 'nrx': 2, 'nbb': 0}
#   elif data_type == 'tx_rx_iq0':
#     data_dict = {'rx0': iq0_capture, 'bb0': iq1_capture, 'nrx': 1, 'nbb': 1}
#   elif data_type == 'iq_all':
#     data_dict = {'rx0': iq0_capture, 'rx1': iq1_capture, 'bb0': iq_tx_capture, 'nrx': 2, 'nbb': 1}
...
#     buffer_uint16 (numpy.ndarray): The buffer containing the raw I/Q data.
#     num_dma_symbol_per_trans (int): Number of DMA symbols per transaction.
#     iq_len (int): The length of the I/Q data in 64-bit words.
...
#   iq_capture   = buffer_reshaped[:, (IQ_CAPTURE_IQ_IDX+0)::WORDS_PER_DMA_SYMBOL] + 1j * buffer_reshaped[:, (IQ_CAPTURE_IQ_IDX+1)::WORDS_PER_DMA_SYMBOL]
#   agc_gain     = buffer_reshaped[:, (IQ_CAPTURE_IQ_IDX+2)::WORDS_PER_DMA_SYMBOL]
#   rssi_half_db = buffer_reshaped[:, (IQ_CAPTURE_IQ_IDX+3)::WORDS_PER_DMA_SYMBOL]
...
#     fd_dict (dict): Dictionary of file descriptors.
#     iq_num_dma_symbol_per_trans (int): Number of DMA symbols per transaction.
#     params (argparse.Namespace): An ArgumentParser object that has been configured with command-line arguments.
#     logger (logging.Logger): A logger object.
...
#   timestamp, lo_freq, trigger_src, capture_all_antenna, tx_start_len = parse_iq_side_info_header(
#     buffer_uint16, iq_num_dma_symbol_per_trans, params.iq_len, params.data_type)
...
#         save_data(fd_dict[f"{AGC}_fd"], data_dict["agc_gain"][trans, :])
#         save_data(fd_dict[f"{RSSI}_fd"], data_dict["rssi_half_db"][trans, :])
#       elif params.data_type in ["rx_iq0_iq1", "tx_rx_iq0", "iq_all"]:
#         for key in ["rx0", "rx1", "bb0"]:
#           if key in data_dict:
#             save_complex_data(fd_dict[f"{key.upper()}_REAL}_fd"], fd_dict[f"{key.upper()}_IMAG}_fd"], data_dict[key][trans, :])
...
# def process_and_save_csi(data, fd_dict, params, logger=None):
#   """Process and save CSI data.

#   Args:
#     data (bytes): Raw CSI data.
#     fd_dict (dict): Dictionary of file descriptors for saving data.
#     params (argparse.Namespace): Parameters for processing.
#     logger (logging.Logger, optional): Logger for debugging.

#   Returns:
#     tuple: A tuple containing:
#       - int: Number of frames.
#       - dict: Processed CSI data.
#   """
#   buffer_uint16 = np.frombuffer(data, dtype='uint16')
...
#   Args:
#     data (bytes): Raw data containing the data type information.

#   Returns:
#     int: The data type index (0 for CSI, 1 for IQ).
#   """
#   data_type_byte = BYTES_PER_DMA_SYMBOL + DATA_BIT_BYTE_IDX
#   data_type_bit = data[data_type_byte]
#   return data_type_bit >> (BITS_PER_BYTE - 1)
...
#   Args:
#     params (argparse.Namespace): Parameters containing iq_len and num_eq.

#   Returns:
#     tuple: A tuple containing:
#       - int: Number of DMA symbols per transaction for IQ data.
#       - int: Number of DMA symbols per transaction for CSI data.

#   Raises:
#     ValueError: If the calculated number of DMA symbols exceeds 8187.
#   """
#   if params.data_type == 'iq_all':
#     iq_num_dma_symbol_per_trans = IQ_HEADER_LEN_DMA_SYM + params.iq_len + (params.iq_len // 2) + 1
#   else:
#     iq_num_dma_symbol_per_trans = IQ_HEADER_LEN_DMA_SYM + params.iq_len

#   csi_num_dma_symbol_per_trans = CSI_HEADER_LEN_DMA_SYM + CSI_LEN_DMA_SYM + params.num_eq * EQUALIZER_LEN_DMA_SYM

#   max_allowed_dma_symbols = 8187  # Max UDP 65507 bytes; (65507/8)-1 = 8187
#   if iq_num_dma_symbol_per_trans > max_allowed_dma_symbols or csi_num_dma_symbol_per_trans > max_allowed_dma_symbols:
#     raise ValueError(f'Limit params.iq_len or num_eq to keep total DMA symbols <= {max_allowed_dma_symbols}')
...
#   if data_type_idx == DATA_TYPE_LIST['iq']:
#     if len(queue_data) % iq_bytes_per_trans != 0:
#       msg = f"Abnormal IQ data length: {len(queue_data)} expected multiple of: {iq_bytes_per_trans}"
#       print(msg)
#       if logger is not None:
#         logger.error(msg)
#       return True
...
#   elif data_type_idx == DATA_TYPE_LIST['csi']:
#     if len(queue_data) % csi_bytes_per_trans != 0:
#       msg = f"Abnormal CSI data length: {len(queue_data)} expected multiple of: {csi_bytes_per_trans}"
#       print(msg)
#       if logger is not None:
#         logger.error(msg)
#       return True
